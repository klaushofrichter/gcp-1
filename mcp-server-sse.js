...
import { getRandomQuote, formatQuoteAsText } from './lib/quote-functions.js';
...
function createMcpServer() {
  const server = new McpServer({
    name: 'quotes-server-sse',
    version: '1.0.0',
  });

  // Register all MCP resources and tools using shared functions
  registerMcpResources(server, quotesData, 'SSE');
  registerMcpTools(server, quotesData, 'SSE');
...
app.get('/', (req, res) => {
  const additionalInfo = {
    description: 'MCP server providing Star Trek quotes via Server-Sent Events (SSE) transport',
    activeSessions: Object.keys(transports).length,
    apiKeyRequired: requiresApiKey(),
    environment: requiresApiKey() ? 'Production' : 'Local Development',
    endpoints: {
      ...
      'GET /': 'API information',
      'GET /pushRandomQuote': 'Trigger random quote event to all connected MCP clients'
    }
  };
  ...
  const apiInfo = getMcpServerMetadata(
    'Star Trek Quotes MCP Server (SSE)',
    'mcp-sse',
    quotesData,
    additionalInfo
  );
...
// Event trigger endpoint - sends random quote to all connected MCP clients
app.get('/pushRandomQuote', async (req, res) => {
  try {
    ...
    if (activeTransports.length === 0) {
      return res.json({
        success: false,
        message: 'No active MCP sessions found',
    ...
    for (const transport of activeTransports) {
      if (typeof transport.sendNotification === 'function') {
        try {
          await transport.sendNotification('quotes/random-quote-pushed', {
          ...
          console.error('Error sending notification to transport:', error);
        }
      }
    }
...
app.post('/register', (req, res) => {
  res.json({
    name: 'Star Trek Quotes MCP Server (SSE)',
    version: '1.0.0',
    description: 'MCP server providing Star Trek quotes via SSE transport',
    transport: 'mcp-sse',
    endpoints: {
      'POST /mcp': 'MCP client requests and initialization',
      'GET /mcp': 'MCP server-to-client notifications (SSE)',
...
app.listen(port, '127.0.0.1', () => {
  console.log('🚀 Star Trek Quotes MCP Server (Server-Sent Events)');
  ...
  console.log('  GET /pushRandomQuote      - Trigger random quote event');
  console.log('\n🔧 MCP Resources:');
  ...
  console.log('\n🎲 Event Trigger:');
  console.log('  curl http://localhost:'+port+'/pushRandomQuote');
});
